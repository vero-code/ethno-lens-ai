...
export function initializeDesignPanel(sandboxProxy, isMockMode, supabase) {
  ...
  designPanel.scanButton.addEventListener('click', async () => {
    ...
    try {
      ...
      const isImage =
        ...
        description.toLowerCase().includes('bitmap');
      if (isImage) {
        showDesignError(
          designPanel,
          "⚠️ You selected an image. Please switch to the 'Image' tab to analyze visuals.",
        );
        return;
      }
  ...
  designPanel.chat.sendButton.addEventListener('click', async () => {
    ...
    // Show cancel button
    designPanel.chat.sendButton.style.display = 'none';
    designPanel.chat.cancelChatButton.style.display = 'inline-flex';
    ...
    try {
      ...
      if (err.name === 'AbortError') {
        designPanel.chat.responseContent.innerHTML = `<span class="info">${MESSAGES.CHAT_CANCELLED}</span>`;
      } else {
