...
const toggleDropzoneContent = (imagePanel, show) => {
  ...
  const message = imagePanel.uploadInput.querySelector(
...
const updatePanelState = (imagePanel, selectedFile) => {
  ...
  const hasAnyInput =
    !!country ||
    (businessType && businessType !== OTHER_OPTION_VALUE) ||
    (businessType === OTHER_OPTION_VALUE &&
      !!imagePanel.otherBusinessInput.value.trim()) ||
    !!selectedFile;
...
export function initializeImagePanel(isMockMode) {
  const imagePanel = {
    ...
    imageOtherBusinessTypeContainer: document.getElementById(
      'imageOtherBusinessTypeContainer',
    ),
    premiumUpsellImage: document.getElementById('premiumUpsellImage'),
    notifyPremiumButtonImage: document.getElementById(
      'notifyPremiumButtonImage',
    ),
  ...
  const resetImagePanel = () => {
    ...
    imagePanel.spinner.style.display = 'none';
    if (imagePanel.cancelAnalyzeButton)
  ...
  const handleFileSelect = (file) => {
    if (file && file.type.startsWith('image/')) {
      ...
      reader.onload = function (e) {
        ...
        if (imagePanel.clearImageButton)
          imagePanel.clearImageButton.style.display = 'block';
        imagePanel.error.style.display = 'none';
  ...
  const handleStep1Change = () => {
    handleBusinessTypeChange(
      imagePanel.businessTypeSelect,
      imagePanel.imageOtherBusinessTypeContainer,
    );
    updatePanelState(imagePanel, selectedFile);
  ...
  imagePanel.notifyPremiumButtonImage.addEventListener('click', async () => {
    ...
    handlePremiumClick(
      imagePanel.notifyPremiumButtonImage,
      userId,
      logPremiumInterest,
      MESSAGES,
    );
  ...
  imagePanel.analyzeButton.addEventListener('click', async () => {
    ...
    let stepIndex = 0;
    imagePanel.resultContent.innerHTML = `<div style="text-align:center; margin-top:20px; color:#666;">${LOADING_STEPS[0]}</div>`;
    let loadingInterval = setInterval(() => {
      stepIndex = (stepIndex + 1) % LOADING_STEPS.length;
      imagePanel.resultContent.innerHTML = `<div style="text-align:center; margin-top:20px; color:#666;">${LOADING_STEPS[stepIndex]}</div>`;
    }, 2000);
    ...
    try {
      ...
      if (!businessType)
        return showImageError(imagePanel, MESSAGES.SELECT_BUSINESS_TYPE);

      let data;
      if (isMockMode()) {
        data = {
          result:
            'This is a **mock response** for the image panel. The real API call was not made.',
        };
      } else {
      ...
      renderMarkdown(
        imagePanel.resultContent,
        data.result,
        '<b>AI Image Analysis</b><br>',
      );
